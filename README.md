...
- **android逆向相关**
    ...
    - android加壳
        - 说一说每一代android加壳的特点 比如：一代壳是怎么加密的？二代在一代的基础上是什么思路？
            - 一代壳最大的特点是dex整体加密，思路比较简单，就是在静态的情况下不让你看见整个dex文件，在内存中动态生成dex，然后自定义类加载器进行加载，运行。这种脱壳的核心思路就是在内存中找到一个比较合适的时机，对dex文件进行dump。
            - 二代壳主要是在一代的基础上增加了代码抽取。这样在你面前呈现出来的代码很多函数都是空的。那是因为在此之前你的dex文件中很多关于method_id部分中代码用00来填充，真正的代码隐藏在别处，在合适的时机，将其恢复成正确的dex。
        - 说一说常见的脱壳思路
            - hook 一些关键函数 open memcpy mmap
            - 修改android源码脱壳 比如dex2oat
